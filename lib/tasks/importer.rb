class Importer
  ...
  def import_articoli
    ...
    for i in 0...articoli.count do
      ...
      testo = "<p><img src='/img/copertine/articoli_#{anno}.jpg' style='width: 100%;'></p><h1>#{titolo}</h1><h2>da #{r.get('old_testata')}</h2><p><span class='data'>del #{ I18n.localize r.get('data'), format: :data_lunga }</span>#{solo_testo}<p><span class='autore'>#{r.get('autore')}</span></p><p><span class='fonte'><a href='http://#{r.get('links')}'>http://#{r.get('links')}</a></span></p>"
